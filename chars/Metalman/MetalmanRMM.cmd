...
value = ifelse(NumHelper(1398),1400,ifelse(random<=500,1400,1300+random%3*10))
...
ignorehitpause = 0
value = ifelse(random<500,440,ifelse(power<1000,1400,3000+random%2*100))

[State -1, AI Normal Attacks 4]
type = ChangeState
triggerall = (roundstate = 2) && (AiLevel) && random <= (1000 * (AIlevel ** 2 / 64.0))
trigger1 = (stateno = 440) && movehit
value = ifelse(power<1000,ifelse(random<500,450,250),3000+random%2*100)

[State -1, AI Hyper]
type = ChangeState
triggerall = (roundstate = 2) && (AiLevel) && random <= (1000 * (AIlevel ** 2 / 64.0))
trigger1 = stateno = 450 && MoveHit 
value = ifelse(power>=1000,3000+random%2*100,1310)
ignorehitpause = 1
...
value = ifelse((p2bodydist X <=50),600,ifelse(NumHelper(1398),1400,ifelse(random<200,1400,1360+random%2*10)));ifelse (random <=500,600,630)
...
value = ifelse(power>=1000,3600+random%2*100,ifelse(random<300,620,ifelse(random<600,1500,1360)))
...
trigger1 = (stateno = 640)
trigger1 = (prevstateno = 610)
value = ifelse(power>=1000,3600+random%2*100,ifelse(random<500,650,1350+random%3*10))
...
;type = ChangeState
;value = 6300
;triggerall = command = "guardpush" && statetype = S && !AiLevel
;trigger1 = stateno = [150,153]
;
;[State -1, Guard Push crouch]
;type = ChangeState
;value = 6310
;triggerall = command = "guardpush" && statetype = C && !AiLevel
;trigger1 = stateno = [150,153]
;
;[State -1, Guard Push aerial]
;type = ChangeState
;value = 6320
;triggerall = command = "guardpush" && statetype = A && !AiLevel
;trigger1 = stateno = [154,155]
